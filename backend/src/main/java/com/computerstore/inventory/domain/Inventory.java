...
public class Inventory {
    ...
    public void adjust(int quantity) { if (availableQuantity + quantity < 0) throw new IllegalArgumentException("Stock cannot be negative."); availableQuantity += quantity; updatedAt = Instant.now(); }
    ...
    public void release(int quantity) { if (reservedQuantity < quantity) throw new IllegalStateException("Reserved stock cannot be negative."); reservedQuantity -= quantity; availableQuantity += quantity; updatedAt = Instant.now(); }
