...
public class CatalogAdminController {
    ...
    @PostMapping("/products") @Transactional public ResponseEntity<ProductSummaryResponse> createProduct(@Valid @RequestBody CreateProductRequest request) { validateVariantNames(request.variants()); if (request.variants().stream().anyMatch(item->item.id()!=null)) throw new InvalidRequestException("New product variants cannot include an id."); Product p=products.save(new Product(request.name().trim(),request.slug().trim(),request.description().trim(),request.price(),category(request.categoryId()),brand(request.brandId()))); saveSpecifications(p, request.specifications()); createVariants(p, request.variants()); return ResponseEntity.status(HttpStatus.CREATED).body(product(p)); }
