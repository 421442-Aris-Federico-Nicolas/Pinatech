...
public class CatalogAdminController {
    private final ProductRepository products; private final CategoryRepository categories; private final BrandRepository brands; private final InventoryRepository inventory; private final ProductImageService productImages;
    public CatalogAdminController(ProductRepository products, CategoryRepository categories, BrandRepository brands, InventoryRepository inventory, ProductImageService productImages) { this.products=products; this.categories=categories; this.brands=brands; this.inventory=inventory; this.productImages=productImages; }
    ...
    private ProductSummaryResponse product(Product p) { return new ProductSummaryResponse(p.getId(),p.getName(),p.getSlug(),p.getDescription(),p.getPrice(),p.getCategory().getId(),p.getCategory().getName(),p.getBrand().getId(),p.getBrand().getName(),productImages.responsesForProduct(p.getId())); }
}
