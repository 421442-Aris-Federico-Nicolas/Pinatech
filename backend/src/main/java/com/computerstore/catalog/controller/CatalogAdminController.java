...
public class CatalogAdminController {
    ...
    @DeleteMapping("/categories/{id}") @ResponseStatus(HttpStatus.NO_CONTENT) @Transactional public void deleteCategory(@PathVariable Long id) { homeConfigurationLock.lock(); Category c=category(id); if (homeSections.existsByCategories_Id(id)) throw new BusinessRuleException("Category cannot be deleted because it is referenced by a home section, including inactive sections."); if (products.existsByCategory_IdAndActiveTrue(id)) throw new BusinessRuleException("Category cannot be deleted while it has active products."); c.deactivate(); categories.save(c); }
    ...
    private ProductSummaryResponse product(Product p) { return new ProductSummaryResponse(p.getId(),p.getName(),p.getSlug(),p.getDescription(),p.getPrice(),p.getCategory().getId(),p.getCategory().getName(),p.getBrand().getId(),p.getBrand().getName(),p.getShippingWeightGrams(),p.getShippingHeightCm(),p.getShippingWidthCm(),p.getShippingLengthCm(),p.getShippingClassificationId(),p.isMustKeepVertical(),productImages.responsesForProduct(p.getId()),specificationResponses(p.getId()),variantResponses(p.getId())); }
}
