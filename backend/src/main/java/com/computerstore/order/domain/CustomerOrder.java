...
@Entity @Table(name="customer_orders") public class CustomerOrder {
 ...
 public CustomerOrder(UserAccount user,List<OrderItem> items,BigDecimal total){this.user=user;this.status=OrderStatus.PENDING_PAYMENT;this.subtotal=total;this.total=total;items.forEach(this::addItem);}
 ...
 public Long getId(){return id;} public OrderStatus getStatus(){return status;} public BigDecimal getTotal(){return total;} public Instant getCreatedAt(){return createdAt;} public List<OrderItem> getItems(){return List.copyOf(items);} public UserAccount getUser(){return user;}
