...
export class LoginComponent {
  private readonly fb = inject(NonNullableFormBuilder); private readonly auth = inject(AuthService); private readonly router = inject(Router);
  ...
  submit(): void { if (this.form.invalid) { this.form.markAllAsTouched(); return; } this.auth.login(this.form.getRawValue()).subscribe({ next: () => void this.router.navigateByUrl(this.auth.user()?.roles.includes('ADMIN') ? '/admin' : this.auth.user()?.roles.includes('TECHNICIAN') ? '/technical' : '/'), error: () => this.error.set('Credenciales inválidas.') }); }
}
