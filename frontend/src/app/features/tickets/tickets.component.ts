...
export class TicketsComponent {
  ...
  async selectTicketImages(ticket: Ticket, files: File[]): Promise<void> {
    if (ticket.status === 'CANCELLED' || ticket.status === 'DELIVERED' || this.uploadingTicket() === ticket.id || this.processingImages()) return;
  ...
  uploadToTicket(ticket: Ticket): void {
    ...
    if (ticket.status === 'CANCELLED' || ticket.status === 'DELIVERED' || !images.length || this.uploadingTicket() !== null || this.processingImages()) return;
    this.clearMessages();
